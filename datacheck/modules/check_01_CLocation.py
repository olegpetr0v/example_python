...
class test10_13():
    def __init__(self, checkFiles):
        ...
        self.cLocationSumJob = 0
        self.cLocationSumDay = 0
        self.cLocationSumMove = 0
        #
    ...
    def update(self, chunk, checkFiles):
        if self.locationExist is True:
            self.cLocationSumHome += chunk.customers_cnt_home.sum()
            self.cLocationSumJob += chunk.customers_cnt_job.sum()
            self.cLocationSumDay += chunk.customers_cnt_day.sum()
            self.cLocationSumMove += chunk.customers_cnt_move.sum()
        #
    ...
    def commit(self):
        if self.locationExist is not True:
            self.result10 = 'Перекрёстне проверки №3 - не проводилось. (Отсутствует отчёт типа Location)'
            self.result11 = 'Перекрёстне проверки №3 - не проводилось. (Отсутствует отчёт типа Location)'
            self.result12 = 'Перекрёстне проверки №3 - не проводилось. (Отсутствует отчёт типа Location)'
            self.result13 = 'Перекрёстне проверки №3 - не проводилось. (Отсутствует отчёт типа Location)'
        else:
            if self.cLocationSumHome == self.locationSumHome:
                self.result10 = 'Перекрёстне проверки №3 - принято.'
            else:
                self.result10 = 'Перекрёстне проверки №3 - отклонено.'
            #
            if self.cLocationSumJob == self.locationSumJob:
                self.result11 = 'Перекрёстне проверки №3 - принято.'
            else:
                self.result11 = 'Перекрёстне проверки №3 - отклонено.'
            #
            if self.cLocationSumDay == self.locationSumDay:
                self.result12 = 'Перекрёстне проверки №3 - принято.'
            else:
                self.result12 = 'Перекрёстне проверки №3 - отклонено.'
            #
            if self.cLocationSumMove == self.locationSumMove:
                self.result13 = 'Перекрёстне проверки №3 - принято.'
            else:
                self.result13 = 'Перекрёстне проверки №3 - отклонено.'
...
def runCheck(filePath, fileDate, checkFiles, extraFiles, counter, lock, fileType=const.T_01_CLOCATION):
    ans =  'Проверка файла %s:' % getFileName(filePath)
    ans += '\r\n'
    ans += '1.2.1  ' + checkName(filePath, fileType)               + '\r\n'
    ans += '1.2.2  ' + checkMD5(filePath, fileType, counter, lock) + '\r\n'
    ans += '1.2.3  ' + checkColumns(filePath, COLUMNS)             + '\r\n'
    ...
    ans += '1.2.4  ' + c4.getResult()           + '\r\n'
    ans += '1.2.5  ' + c5.getResult()           + '\r\n'
    ans += '1.2.6  ' + c6_7.getResult()[0]      + '\r\n'
    ans += '1.2.7  ' + c6_7.getResult()[1]      + '\r\n'
    ans += '1.2.8  ' + c8.getResult()           + '\r\n'
    ans += '1.2.9  ' + c9.getResult()           + '\r\n'
    ans += '1.2.10 ' + c10_13.getResult10()     + '\r\n'
    ans += '1.2.11 ' + c10_13.getResult11()     + '\r\n'
    ans += '1.2.12 ' + c10_13.getResult12()     + '\r\n'
    ans += '1.2.13 ' + c10_13.getResult13()     + '\r\n'
